...
class HavingSpecMetricComparator
{
  ...
  static int compare(Row row, String aggregationName, Number value)
  {
    ...
    if (metricValueObj == null) {
      throw new ISE("cannot compare null Values");
    }
    if (metricValueObj instanceof Long) {
      ...
      float f = ((Float) metricValueObj).floatValue();
      return Floats.compare(f, value.longValue());
    } else if (metricValueObj instanceof Double) {
      double d = ((Double) metricValueObj).doubleValue();
    ...
    throw new ISE("Unknown Type[%s] to compare", metricValueObj.getClass());
  }
